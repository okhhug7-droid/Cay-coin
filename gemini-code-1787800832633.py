...
@app.route('/')
def home():
    return f"Web API is running! Target: {CAYTIEN_WEB_URL} | Created by {CREATOR_NAME} (Vĩnh Phúc, VN)"
...
def check_status():
    ...
    if user_id in db and isinstance(db[user_id], dict) and db[user_id].get("status") == True:
        ...
        user_data["status"] = False
        write_db(db)
        return jsonify({"success": True, "redirect_url": CAYTIEN_WEB_URL})
...
class LienKetModal(discord.ui.Modal, title="🔗 Liên Kết Tài Khoản Cây Tiền"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.description = (
            ...
            f"🌐 **Trang quản lý:** [Cây Tiền]({CAYTIEN_WEB_URL})"
        )
        embed.set_footer(text=f"{CREATOR_NAME} | Vĩnh Phúc, VN")
                    ...
                    if data.get("success"):
                                ...
                                if log_channel:
                                    log_embed = discord.Embed(
                                        ...
                                        description=(
                                            ...
                                            f"🔗 **Web quản lý:** [Cây Tiền]({CAYTIEN_WEB_URL})"
                                        ),
                        ...
                        private_embed = discord.Embed(
                            ...
                            description=(
                                ...
                                f"✅ Tiền đã được cộng vào hệ thống web của bạn tại: [Cây Tiền]({CAYTIEN_WEB_URL})"
                            ),
                            color=discord.Color.green()
                        )
...
async def sodu(interaction: discord.Interaction):
    ...
    embed.add_field(name="🌐 Quản lý ví web", value=f"[Truy cập Cây Tiền]({CAYTIEN_WEB_URL})", inline=False)
