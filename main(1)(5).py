...
def create_xp_card(member: discord.Member, xp: int, level: int, xp_needed: int) -> str:
    """Tạo card XP nền trong suốt/đen tối giản, có avatar Discord."""
    try:
        ...
        def load_font(size, bold=False):
            for fp in (font_candidates if bold else regular_candidates):
        ...
        draw.rounded_rectangle((35, 35, 675, 465), radius=32,
                               fill=(8, 8, 8), outline=(255, 255, 255), width=2)

        name = member.display_name[:24]
        draw.text((70, 65), name, font=title_font, fill=(255, 255, 255))
        draw.text((70, 130), f"LEVEL {level} / 300", font=level_font, fill=(235, 235, 235))

        ratio = 1.0 if level >= 300 else min(max(xp / max(xp_needed, 1), 0), 1)
        bar_x1, bar_y1, bar_x2, bar_y2 = 70, 210, 620, 252
        draw.rounded_rectangle((bar_x1, bar_y1, bar_x2, bar_y2), radius=20, fill=(55, 55, 55))
        fill_x2 = bar_x1 + int((bar_x2 - bar_x1) * ratio)
        if fill_x2 > bar_x1:
            draw.rounded_rectangle((bar_x1, bar_y1, fill_x2, bar_y2), radius=20, fill=(255, 255, 255))

        current_text = f"{xp:,} / {xp_needed:,} XP" if level < 300 else f"{xp:,} XP • MAX"
        draw.text((70, 275), current_text, font=xp_font, fill=(255, 255, 255))
        draw.text((70, 345), f"Tiến trình: {ratio * 100:.1f}%", font=small_font, fill=(225, 225, 225))

        cx, cy, r = 875, 250, 165
        box = (cx-r, cy-r, cx+r, cy+r)
        draw.ellipse(box, outline=(60, 60, 60), width=28)
        if ratio > 0:
            draw.arc(box, start=-90, end=-90 + int(360 * ratio), fill=(255, 255, 255), width=28)

        xp_inside = f"{xp:,}" if level < 300 else "MAX"
        bbox = draw.textbbox((0, 0), xp_inside, font=xp_font)
        draw.text((cx - (bbox[2]-bbox[0])/2, cy - (bbox[3]-bbox[1])/2 - 5), xp_inside, font=xp_font, fill=(255, 255, 255))
        small = "XP"
        bbox = draw.textbbox((0, 0), small, font=small_font)
        draw.text((cx - (bbox[2]-bbox[0])/2, cy + 52), small, font=small_font, fill=(225, 225, 225))

        try:
            avatar_data = urllib.request.urlopen(str(member.display_avatar.url), timeout=5).read()
...
class ThongBaoModal(discord.ui.Modal, title="📢 TẠO THÔNG BÁO"):
    message = discord.ui.TextInput(label="Nội dung thông báo", placeholder="Nhập nội dung cần gửi...", style=discord.TextStyle.paragraph, required=True, max_length=4000)
    poll_question = discord.ui.TextInput(label="Câu hỏi bình chọn", placeholder="Để trống nếu không cần bình chọn", required=False, max_length=256)
    poll_options = discord.ui.TextInput(label="Các lựa chọn", placeholder="Ví dụ: Có, Không, Chưa chắc", required=False, max_length=1000)

    def __init__(self, channel: discord.TextChannel, media: discord.Attachment = None):
        super().__init__()
        self.channel = channel
        self.media = media
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = make_embed(title="📢 THÔNG BÁO", description=self.message.value.strip(), color=discord.Color.blurple())
        file = None
        try:
            if self.media:
                file = await self.media.to_file()
                if self.media.content_type and self.media.content_type.startswith("image/"):
                    embed.set_image(url=f"attachment://{self.media.filename}")
                else:
                    embed.add_field(name="🎬 Tệp đính kèm", value=self.media.filename, inline=False)

            poll = None
            if self.poll_question.value.strip():
                options = [x.strip() for x in self.poll_options.value.split(",") if x.strip()]
                if not 2 <= len(options) <= 10:
                    await interaction.response.send_message("❌ Bình chọn cần từ 2 đến 10 lựa chọn, ngăn cách bằng dấu phẩy.", ephemeral=True)
                    return
                poll = discord.Poll(question=self.poll_question.value.strip(), duration=datetime.timedelta(days=1), multiple=False)
                for option in options:
                    poll.add_answer(text=option)

            await self.channel.send(content=f"<@&{role_id}>", embed=embed, file=file, poll=poll, allowed_mentions=discord.AllowedMentions(roles=True))
            await interaction.response.send_message(f"✅ Đã gửi thông báo vào {self.channel.mention}!", ephemeral=True)
        except Exception as e:
            print(f"⚠️ Lỗi gửi thông báo: {e}")
            await interaction.response.send_message("❌ Không thể gửi thông báo. Kiểm tra quyền bot và tệp đính kèm.", ephemeral=True)


@bot.tree.command(name="thongbao", description="Mở modal tạo thông báo có thể kèm bình chọn (Admin)")
@discord.app_commands.checks.has_permissions(administrator=True)
async def thongbao(interaction: discord.Interaction, media: discord.Attachment = None):
    db_cursor.execute("SELECT channel_id FROM announcement_channels WHERE guild_id = ?", (interaction.guild.id,))
    row = db_cursor.fetchone()
    if not row:
        await interaction.response.send_message("❌ Chưa cài kênh thông báo. Dùng `/setannouncement` trước.", ephemeral=True)
        return
    channel = interaction.guild.get_channel(row[0])
    if not channel:
        await interaction.response.send_message("❌ Không tìm thấy kênh thông báo. Hãy cài lại bằng `/setannouncement`.", ephemeral=True)
        return
    await interaction.response.send_modal(ThongBaoModal(channel, media))
