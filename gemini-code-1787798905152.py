...
CAYTIEN_WEB_URL = "https://caytien.site/dashboard.php" #
DB_FILE = 'database.json'
CREATOR_NAME = "to by ph.huyy"
CONFIG_FILE = 'config.json'

# Giới hạn số lần làm mỗi ngày cho từng dịch vụ (Đã xóa octolink)
DAILY_LIMITS = {
    "link4m": 2,
    "bbmkts": 1,
    "site2s": 2,
...
# Cấu hình chuẩn xác cho từng API (Đã xóa octolink)
API_CONFIGS = {
    "link4m": {
...
# Lệnh mới giúp người dùng liên kết tài khoản web
@bot.tree.command(name="lienket", description="Liên kết tài khoản website Cây Tiền của bạn với Discord")
@app_commands.describe(username="Tên đăng nhập hoặc Email tài khoản của bạn trên website Cây Tiền")
async def lienket(interaction: discord.Interaction, username: str):
    ...
    embed.description = (
        f"Đã liên kết thành công tài khoản Discord của bạn với tên đăng nhập web:\n"
...
async def nhancoin(interaction: discord.Interaction):
    ...
    # Kiểm tra xem người dùng đã liên kết tài khoản web chưa
    users_data = read_db()
    if user_id not in users_data or not users_data[user_id].get("web_username") or users_data[user_id].get("web_username") == "Chưa liên kết":
        return await interaction.response.send_message("❌ Bạn chưa liên kết tài khoản web! Vui lòng dùng lệnh `/lienket <tên_đăng_nhập_web>` trước khi kiếm coin.", ephemeral=True)
...
async def sodu(interaction: discord.Interaction):
    ...
    if user_id not in users_data or not isinstance(users_data[user_id], dict):
        return await interaction.response.send_message("❌ Bạn chưa có số dư coin nào! Hãy dùng lệnh `/lienket` và `/nhancoin` để bắt đầu.", ephemeral=True)
