        ...
        try:
            if config["method"] == "GET":
                ...
                async with session.get(api_url) as resp:
                    ...
                    print(f"🔍 DEBUG [{service_name}] Response: {text_res_clean[:200]}")
                    ...
                    if 'application/json' in content_type or text_res_clean.startswith("{") or text_res_clean.startswith("["):
                        try:
                            ...
                            if short_link and isinstance(short_link, str) and short_link.startswith("http") and destination_url not in short_link:
