...
async def shorten_with_api(service_name, destination_url):
    """Tạo link vượt và không để Cloudflare 5xx lọt vào Discord."""
    ...
    def find_short_urls(obj):
        """Trả về tất cả URL hợp lệ, bỏ Cloudflare thay vì chỉ lấy URL đầu tiên."""
        found = []
        ...
        if isinstance(obj, str):
            ...
            preferred = (
                ...
                "shortUrl", "short", "link", "url", "result", "destination"
            )
            # Ưu tiên các field thường chứa shortlink.
            for key in preferred:
                if key in obj:
                    for item in find_short_urls(obj[key]):
                        if item not in found:
                            found.append(item)
            # Quét toàn bộ JSON nếu field chuẩn không có.
    ...
    def choose_octolink(urls):
        for url in urls:
            if not is_valid_octolink(url):
                continue
            return url
        return None
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        try:
            ...
            if method == "GET":
                ...
                # Octolink: đúng dạng API bạn yêu cầu.
                ...
                async with session.get(
                    ...
                    # 1. Nếu API trả Location, chỉ nhận Location hợp lệ.
                    location = valid_http_url(resp.headers.get("Location"))
                    if location:
                        if service_name == "octolink":
                            chosen = choose_octolink([location])
                            if chosen:
                                return chosen
                            print(f"⚠️ Octolink Location không phải link Octolink: {location}")
                        else:
                            return location

                    # 2. Quét response text, bỏ Cloudflare và lấy URL hợp lệ khác.
                    urls = find_short_urls(text_clean)

                    if service_name == "octolink":
                        chosen = choose_octolink(urls)
                        if chosen:
                            return chosen
                    elif urls:
                        return urls[0]

                    # 3. Nếu response là JSON, quét sâu toàn bộ JSON.
                    try:
                    ...
                    if "cloudflare.com/5xx-error-landing" in text_clean.lower():
                        print(
                            f"⚠️ [{service_name}] API có Cloudflare 5xx; "
                            "đã loại URL lỗi và không gửi nó cho Discord."
                        )
