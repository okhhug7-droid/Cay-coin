        ...
        try:
            ...
            if method == "GET":
                if "bbmkts.com" in base_url:
                    ...
                    params = {param_name: destination_url}
                elif service_name == "octolink":
                    # Octolink API: /api?api=API_KEY&url=DESTINATION&alias=ALIAS
                    # Dùng alias ngẫu nhiên để tránh trùng alias giữa các lượt.
                    alias = "ca" + ''.join(
                ...
                async with session.get(
                    ...
                    # Ưu tiên parse JSON để xử lý đúng response của Octolink.
                    data = None
                    try:
                    ...
                    if data is not None:
                        ...
                        if found:
                            # Không ép hostname của URL trả về phải là octolink.vip;
                            # một số phiên bản API có thể trả về domain shortlink khác.
                            return found
                        print(f"⚠️ [{service_name}] JSON response không chứa URL: {text_clean[:500]}")
                        return None
