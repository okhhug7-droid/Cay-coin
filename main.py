...
def save_user():
    ...
    if user_id not in db or not isinstance(db[user_id], dict):
        db[user_id] = {"total_completed": 0, "status": False}
    else:
...
def complete():
    ...
    if user_id in db and isinstance(db[user_id], dict):
        ...
        write_db(db)
        return "<h1>🎉 Vượt link thành công! Bạn có thể quay lại Discord để nhận thưởng.</h1>"
...
async def nhancoin(interaction: discord.Interaction):
    ...
    # 1. Lưu trạng thái chờ của user qua Flask API nội bộ
    ...
    # 2. Tạo mã ngẫu nhiên để chống cache link4m
        ...
        try:
            async with ClientSession() as session:
                async with session.get(f"{api_local_url}/check-status?user_id={user_id}") as resp:
                    data = await resp.json()
                    if data.get("success"):
                        await interaction.followup.send(f"🎉 Chúc mừng {interaction.user.mention}! Bạn đã vượt link thành công và nhận được phần thưởng!", ephemeral=True)
                        return
        except:
            pass
...
if __name__ == '__main__':
    ...
    if not token:
        ...
        # Chạy Flask web server ở một luồng riêng (background thread)
        flask_thread = Thread(target=run_flask)
        flask_thread.daemon = True
        flask_thread.start()
        print("🌐 Flask Web Server đã khởi chạy ngầm thành công!")

        # Chạy Discord Bot chính
        bot.run(token)
