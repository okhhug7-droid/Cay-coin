            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{api_local_url}/check-status?user_id={user_id}&service={chosen_service}") as resp:
                    ...
                    if data.get("success"):
                        ...
                        # Thông báo riêng cho người dùng: Coin nhận đúng theo dịch vụ đã chọn.
                        success_embed = discord.Embed(
                            title="🎉 VƯỢT LINK THÀNH CÔNG!",
                            description=(
                                f"Chúc mừng {interaction.user.mention}! Bạn đã hoàn thành link thành công.\n\n"
                                f"🛠️ **Dịch vụ:** `{chosen_service.upper()}`\n"
                        ...
                        success_embed.set_footer(
                            text=f"{CREATOR_NAME} • {get_vietnam_time().strftime('%d/%m/%Y %H:%M:%S')}"
                        )
