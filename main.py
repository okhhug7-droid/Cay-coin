...
async def shorten_with_api(service_name, destination_url):
    ...
    def is_bad_url(url):
        """Không bao giờ cho URL Cloudflare 5xx lọt ra ngoài bot."""
        if not isinstance(url, str):
            return True
        u = url.strip().lower()
        return (
            "cloudflare.com/5xx-error-landing" in u
            or "www.cloudflare.com/5xx-error-landing" in u
        )
        ...
        try:
            ...
            if method == "GET":
                ...
                if service_name == "octolink":
                    # Alias phải khác nhau giữa các lượt để tránh trùng alias.
                    params["alias"] = "CA" + ''.join(random.choices(string.ascii_letters + string.digits, k=10))
                ...
                async with session.get(
                    ...
                    if location and location.startswith(("http://", "https://")):
                        if is_bad_url(location):
                            print(f"⚠️ [{service_name}] API trả redirect Cloudflare 5xx -> bỏ qua.")
                            return None

                        # Chỉ nhận URL HTTP(S) thực sự; không follow redirect ở phía bot.
                    ...
                    if "cloudflare.com/5xx-error-landing" in text_clean.lower():
                        print(f"⚠️ [{service_name}] Response chứa Cloudflare 5xx -> không gửi URL này cho Discord.")

                    try:
                        data = json.loads(text_clean)
                        found = find_short_url(data)
                        if found and not is_bad_url(found):
                            return found
