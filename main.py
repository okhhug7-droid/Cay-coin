...
""")
    ...
    # Đọc token từ biến môi trường BOT_TOKEN (phục vụ Railway)
    token = os.environ.get("BOT_TOKEN", "").strip()

    if not token and len(sys.argv) > 1:
        token = sys.argv[1].strip()
    elif not token and os.path.exists(".token"):
        with open(".token", "r") as f:
            token = f.read().strip()

    if not token:
        log("Token trống – Vui lòng cấu hình biến môi trường BOT_TOKEN trên Railway!", "error")
        sys.exit(1)
    ...
    try:
        ...
        log("Môi trường không hỗ trợ input tương tác (Railway worker). Chuyển sang chế độ chạy ngầm mặc định...", "warn")
        choice = "2"
    ...
    if choice == "1":
        ...
        if target_user_id:
            ...
            chan_id = input("Nhập Channel ID muốn gửi: ").strip()
            if chan_id:
                resp = api.post(f"/channels/{chan_id}/messages", {"content": invite_content})
                if resp.status_code in (200, 201):
                    log("Đã gửi lời mời tham gia chuỗi thành công!", "ok")
        else:
            log("User ID không hợp lệ.", "error")
    else:
        log("Lựa chọn không hợp lệ hoặc không được hỗ trợ chạy ngầm.", "error")
