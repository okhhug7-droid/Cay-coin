...
CAYTIEN_WEB_URL = "https://caytien.site/dashboard.php"[cite: 1]
...
def check_user_linked(user_id):
    """Hàm kiểm tra xem user đã liên kết tài khoản web hợp lệ chưa"""
    db = read_db()
    if user_id not in db or not isinstance(db[user_id], dict):
        return False
    web_username = db[user_id].get("web_username", "Chưa liên kết")
    if not web_username or web_username == "Chưa liên kết":
        return False
    return True
...
@app.route('/')
def home():
    return f"Web API is running! Target: {CAYTIEN_WEB_URL} | Created by {CREATOR_NAME} (Vĩnh Phúc, VN)"[cite: 1]
...
def complete():
    ...
    if user_id in db and isinstance(db[user_id], dict):
        ...
        <p>Hệ thống đang cộng tiền vào tài khoản của bạn trên <a href="{CAYTIEN_WEB_URL}" target="_blank">Cây Tiền</a>.</p>
        <p>Bạn có thể quay lại Discord để kiểm tra số dư.</p>
        <hr><small>Hệ thống phát triển bởi {CREATOR_NAME} - Vĩnh Phúc, VN</small>
        """
...
def check_status():
    ...
    if user_id in db and isinstance(db[user_id], dict) and db[user_id].get("status") == True:
        ...
        user_data["status"] = False
        write_db(db)
        return jsonify({"success": True, "redirect_url": CAYTIEN_WEB_URL})[cite: 1]
...
async def shorten_with_api(service_name, destination_url):
    ...
    def is_bad_url(url):
        ...
        return any(x in u for x in bad_hosts)
    ...
    def find_short_urls(obj):
        found = []
        def add(value):
            value = valid_http_url(value)
        ...
        if isinstance(obj, str):
            ...
            preferred = ("shortenedUrl", "shortened_url", "shorturl", "short_url", "shortUrl", "short", "link", "url", "result", "destination")
            for key in preferred:
                if key in obj:
                    for item in find_short_urls(obj[key]):
                        if item not in found:
                            found.append(item)
            for value in obj.values():
                for item in find_short_urls(value):
                    if item not in found:
                        found.append(item)
        elif isinstance(obj, list):
            for item in obj:
                for url in find_short_urls(item):
                    if url not in found:
                        found.append(url)
        return found
        ...
        try:
            ...
            if method == "GET":
                ...
                async with session.get(base_url, params=params, headers=headers, allow_redirects=False) as resp:
                    ...
                    location = valid_http_url(resp.headers.get("Location"))
                    if location:
                        return location

                    urls = find_short_urls(text_clean)
                    if urls:
                        return urls[0]

                    try:
                        data = json.loads(text_clean)
                        urls = find_short_urls(data)
                        if urls:
                            return urls[0]
                ...
                payload = {"url": destination_url, "taskType": config.get("task_type", "review")}
                async with session.post(base_url, headers=headers, json=payload, allow_redirects=False) as resp:
                    text_res = await resp.text()
                    text_clean = text_res.strip()
            ...
            print(f"❌ Lỗi khi gọi API {service_name}: {e}")
...
class ChannelSelectDropdown(discord.ui.Select):
    def __init__(self, guild):
        ...
        options = [discord.SelectOption(label=ch.name, value=str(ch.id), description=f"Chọn kênh #{ch.name} làm kênh thông báo") for ch in text_channels]
        if not options:
...
async def setupkenh(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="⚙️ THIẾT LẬP KÊNH THÔNG BÁO", description="Vui lòng chọn kênh bên dưới từ danh sách để làm nơi gửi thông báo khi có thành viên vượt link thành công:", color=0x38bdf8)
    embed.set_footer(text=f"{CREATOR_NAME} | Vĩnh Phúc, VN")
    await interaction.response.send_message(embed=embed, view=view, ephemeral=True)

# ================= MODAL BẢNG NHẬP LIÊN KẾT TÀI KHOẢN =================
class LienKetModal(discord.ui.Modal, title="🔗 Liên Kết Tài Khoản Cây Tiền"):
    username_input = discord.ui.TextInput(
        label="Tên đăng nhập / Email trên web",
        placeholder="Nhập chính xác tên tài khoản hoặc email của bạn...",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if user_id not in db or not isinstance(db[user_id], dict):
            db[user_id] = {
                ...
                "coins": 0,
                "status": False,
                "last_date": get_current_date_str(),
                "daily_tasks": {}
            }
        else:
            db[user_id]["web_username"] = username
            
        write_db(db)
        
        embed = discord.Embed(title="✅ LIÊN KẾT TÀI KHOẢN THÀNH CÔNG", color=discord.Color.green())
        embed.description = (
            f"Tài khoản Discord của bạn đã được gắn kết với hệ thống web:\n\n"
            f"👤 **Tài khoản Web:** `{username}`\n"
            f"🌐 **Trang quản lý:** [Cây Tiền]({CAYTIEN_WEB_URL})"[cite: 1]
        )
        embed.set_footer(text=f"{CREATOR_NAME} | Vĩnh Phúc, VN")
        await interaction.response.send_message(embed=embed, ephemeral=True)

@bot.tree.command(name="lienket", description="Mở bảng nhập để liên kết tài khoản website Cây Tiền")
async def lienket(interaction: discord.Interaction):
    await interaction.response.send_modal(LienKetModal())

class LinkSelectDropdown(discord.ui.Select):
    def __init__(self, user_id):
        self.user_id = user_id
        self.selected_service = None
        self.earned_coins = 370
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.earned_coins = coin_mapping.get(self.selected_service, 370)
        self.view.stop()
...
async def nhancoin(interaction: discord.Interaction):
    ...
    # 🔒 BẮT BUỘC LIÊN KẾT TRƯỚC
    if not check_user_linked(user_id):
        embed = discord.Embed(
            title="⚠️ CHƯA LIÊN KẾT TÀI KHOẢN",
            description="Bạn cần liên kết tài khoản website trước khi sử dụng tính năng này!\n\n👉 Vui lòng dùng lệnh **`/lienket`** để mở bảng nhập thông tin Cây Tiền.",
                ...
                async with session.get(f"{api_local_url}/check-status?user_id={user_id}&service={chosen_service}") as resp:
                    ...
                    if data.get("success"):
                        ...
                        if user_id in users_data:
                            ...
                            web_username = users_data[user_id].get("web_username", "Chưa liên kết")
                            write_db(users_data)
                                ...
                                if log_channel:
                                    log_embed = discord.Embed(
                                        ...
                                        description=(
                                            ...
                                            f"🌐 **Tài khoản Web:** `{web_username}`\n"
                                            f"🛠️ **Dịch vụ:** `{chosen_service.upper()}`\n"
                                            f"🎁 **Phần thưởng:** `+{reward_coins:,} Coin`\n"
                                            f"💰 **Số dư mới:** `{current_total_coins:,} Coin`\n"
                                            f"🔗 **Web quản lý:** [Cây Tiền]({CAYTIEN_WEB_URL})"[cite: 1]
                                        ),
                        ...
                        private_embed = discord.Embed(
                            ...
                            description=(
                                ...
                                f"✅ Tiền đã được cộng vào hệ thống web của bạn tại: [Cây Tiền]({CAYTIEN_WEB_URL})"[cite: 1]
                            ),
                            color=discord.Color.green()
                        )
...
@bot.tree.command(name="sodu", description="Kiểm tra số dư coin và tài khoản web liên kết của bạn")
async def sodu(interaction: discord.Interaction):
    user_id = str(interaction.user.id)
    
    # 🔒 BẮT BUỘC LIÊN KẾT TRƯỚC
    if not check_user_linked(user_id):
        return await interaction.response.send_message("❌ Bạn chưa liên kết tài khoản web! Vui lòng dùng lệnh **`/lienket`** trước khi kiểm tra số dư.", ephemeral=True)

    users_data = read_db()
    user_info = users_data[user_id]
    total_completed = user_info.get("total_completed", 0)
    total_coins = user_info.get("coins", 0)
    web_username = user_info.get("web_username", "Chưa liên kết")
    ...
    embed.add_field(name="🌐 Tài Khoản Web", value=f"`{web_username}`", inline=False)
    ...
    embed.add_field(name="🌐 Quản lý ví web", value=f"[Truy cập Cây Tiền]({CAYTIEN_WEB_URL})", inline=False)[cite: 1]
...
async def topvuotlink(interaction: discord.Interaction):
    ...
    for idx, (user_id, total) in enumerate(sorted_users[:10]):
        ...
        web_name = info.get("web_username", "Ẩn danh") if isinstance(info, dict) else "Ẩn danh"
        coins = info.get("coins", 0) if isinstance(info, dict) else 0
        rank_display = medal_emojis[idx] if idx < 3 else f"`#{idx+1:02d}`"
        leaderboard_lines.append(f"{rank_display} | <@{user_id}> (`{web_name}`) ➔ **{total}** lần (Tổng: **{coins:,}** Coin)")
